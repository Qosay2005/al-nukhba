...
export default function Footer() {
  return (
    ...
    >
      <div className="mx-auto grid max-w-7xl grid-cols-1 gap-8 text-center md:grid-cols-3 md:text-right">
        ...
        <div className="flex flex-col items-center gap-3 md:items-start">
          ...
          <p className="max-w-md text-sm leading-relaxed text-ivory/60">
            طاقم النخبة — رواد الضيافة الفاخرة وتنظيم أرقى المناسبات والأعراس
            في بلاد الشام والخليج
          </p>
          ...
          <div className="flex flex-wrap justify-center gap-2 md:justify-start">
            ...
            {socialLinks.map(({ label, url, icon: Icon }) => (
              <a
                ...
                rel="noreferrer"
                aria-label={label}
                className="flex items-center gap-2 rounded-md border border-gold-dark/40 bg-bg-base px-3 py-2 text-sm text-ivory/70 transition-colors duration-200 hover:border-gold hover:text-gold"
              >
                <Icon
                  size={17}
                  aria-hidden="true"
      ...
      <div className="mx-auto flex flex-col gap-2 mt-8 max-w-7xl border-t border-gold-dark/20 pt-5 text-center">
        <p className="text-xs text-ivory/50">
          © سفرجية النخبة. جميع الحقوق محفوظة.
        </p>
         <p className="text-xs text-ivory/50">
         تم انشاء الموقع بواسطة المهندس قصي رائد  قلالوة
        </p>
      </div>
